"""
Live Indicators & Signal Generator → PostgreSQL
Reads historical_prices and indicators from PostgreSQL,
...
def process_stock(symbol):
    ...
    if not price_rows:
        print(f"[live_indicators] No price data in PostgreSQL for {symbol}. Run indicators_all.py first.")
        return
    ...
    print(f"[live_indicators] {symbol}: {direction} | "
          ...
          f"(confidence {confidence:.1f}%) → saved to PostgreSQL")
...
def main():
    ...
    print(f"\n[live_indicators] Done at {datetime.now()}. Results in PostgreSQL `predictions`.")
