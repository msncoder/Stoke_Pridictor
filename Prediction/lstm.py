"""
LSTM Stock Price Prediction → PostgreSQL `predictions` table
Reads historical_prices from PostgreSQL, trains LSTM, stores predictions in PostgreSQL.
...
STOCKS     = ["UBL", "PSO", "HBL", "ENGRO", "OGDCL"]
EPOCHS     = 10
BATCH_SIZE = 1
NEURONS    = 1
...
def load_prices(symbol):
    ...
    if not rows:
        print(f"[lstm] No data for {symbol} in PostgreSQL.")
    return rows
...
def save_prediction(symbol, predicted_value, target_period, direction="NEUTRAL"):
    config.execute(
        ...
        VALUES (%s, %s, %s, %s, %s, %s)
        ON CONFLICT (symbol, predicted_at, model_type) DO NOTHING
        """,
...
def process_stock(symbol):
    ...
    for step in range(1, FORECAST + 1):
        ...
        date_label = next_date.strftime('%Y-%m-%d')
        
        save_prediction(symbol, round(future_val, 4), date_label, direction)
        last_obs = future_val
        last_date = next_date
        print(f"[lstm] {symbol} {date_label}: {future_val:.2f} [{direction}] → PostgreSQL")
...
def main():
    ...
    print(f"\n[lstm] All stocks done. Predictions stored in PostgreSQL `predictions`.")
