"""
Actual vs Predicted Tracker → PostgreSQL
...
def print_summary(symbol):
    ...
    stats = config.fetchone(
        ...
        SELECT
            COUNT(*)                              AS total_rows,
            COUNT(actual_value)                   AS filled,
            AVG(ABS(pct_error))                   AS avg_pct_error,
            SUM(CASE WHEN direction = 'BUY'  AND difference > 0 THEN 1 ELSE 0 END) AS correct_buy,
            SUM(CASE WHEN direction = 'SELL' AND difference < 0 THEN 1 ELSE 0 END) AS correct_sell
        FROM predictions
        WHERE symbol = %s
          AND model_type = 'LSTM'
...
def main():
    ...
    print(f"[valupdate] Done at {datetime.now()}. Results in PostgreSQL `predictions`.")
