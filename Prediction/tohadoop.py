...
Reads data from PostgreSQL and uploads to HDFS as Parquet/CSV.
...
def PostgreSQL_to_csv_bytes(table, symbol=None):
    """Serialize a PostgreSQL table to CSV bytes for HDFS upload."""
    if symbol:
...
def upload_table(hdfs, table, hdfs_dir, symbol=None):
    ...
    data = PostgreSQL_to_csv_bytes(table, symbol)
    if data is None:
