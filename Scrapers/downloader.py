"""
Document Downloader & Parser → PostgreSQL `documents` table
Interactively downloads PDFs/web articles, extracts text, stores in PostgreSQL.
...
def save_document(title, body, url, file_type="webpage"):
    """Store document in PostgreSQL, skip duplicates."""
    try:
        config.execute(
            """
            INSERT INTO documents (title, body, source_url, file_type, created_at)
            VALUES (%s, %s, %s, %s, %s)
            ON CONFLICT (title) DO NOTHING
            """,
            (title[:499], body or None, url[:1023] if url else None, file_type, datetime.now()),
        )
        print(f"[downloader] Saved to PostgreSQL: {title[:60]!r}")
    except Exception as e:
...
def search_articles(keyword):
    """Full-text search articles in PostgreSQL."""
    ...
    try:
        rows = config.fetchall(
            ...
            FROM articles
            WHERE to_tsvector('english', title || ' ' || coalesce(body, '')) @@ plainto_tsquery(%s)
...
def interactive_mode():
    ...
    print("=" * 60)
    print("SMAP-FYP Document Downloader → PostgreSQL")
    print("=" * 60)
